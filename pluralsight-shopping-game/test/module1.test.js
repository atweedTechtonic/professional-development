...
try {
  esprima.parseModule(source, {}, function (node) {
    ...
    if (node.type === 'ClassDeclaration' && node.id.name === 'Rating') {
...
describe('Shopping Master game - Define object-types/classes', function () {
  describe('Define the player object', () => {
    it('Should define the player object. @player-object', function () {
      ...
      test.assert(
        shoppinggame.name && shoppinggame.name == 'unknown',
        'Have you defined a constant called `name` & initialised to a value of `unknown`?'
      );
      test.assert(
        shoppinggame.score == 0,
        'Have you defined a constant called `score` & initialised to a value of `0`?'
      );
      test.assert(
        shoppinggame.items == 0,
        'Have you defined a constant called `items` & initialised to a value of `0`?'
      );
      test.assert(
        shoppinggame.player &&
  ...
  describe('Add three methods to the player object', () => {
    it('Should add three methods to the player object. @player-object-methods', function () {
      const methods =
        playerDef &&
        playerDef.init.properties.filter(
          (element) => element.value.type == 'FunctionExpression'
        );

      //console.log('methods', methods);

      test.assert(
        shoppinggame.player && shoppinggame.player.getCurrentScore,
        "Have you added three methods out of which there's a method named `getCurrentScore()` to the `player` object?"
      );
      test.assert(
        shoppinggame.player.addPoints,
        "Have you added three methods out of which there's a method named `addPoints()` to the `player` object?"
      );
      test.assert(
        shoppinggame.player.deductPoints,
        "Have you added three methods out of which there's a method named `deductPoints()` to the `player` object?"
      );
      ...
      for (let i = 0; i < methods.length; i++) {
        if (
          methods[i].key.name == 'getCurrentScore' &&
        ...
        if (
          ...
          if (methods[i].value.body.body[0].expression.operator == '=') {
            thisUsedInaddPoints =
              ...
              methods[i].value.body.body[0].expression.left.object.type ==
                'ThisExpression' &&
              methods[i].value.body.body[0].expression.right.left.object &&
              methods[i].value.body.body[0].expression.right.left.object.type ==
                'ThisExpression'
                ? true
                : false;
          } else if (
            methods[i].value.body.body[0].expression.operator == '+='
          ) {
            thisUsedInaddPoints =
              methods[i].value.body.body[0].expression.left.object &&
              methods[i].value.body.body[0].expression.left.object.type ==
        ...
        if (
          methods[i].key.name == 'deductPoints' &&
          methods[i].value.body.body[0].expression
        ) {
          if (methods[i].value.body.body[0].expression.operator == '=') {
            thisUsedIndeductPoints =
              methods[i].value.body.body[0].expression.left.object &&
              methods[i].value.body.body[0].expression.left.object.type ==
                'ThisExpression' &&
              methods[i].value.body.body[0].expression.right.left.object &&
      ...
      test.assert(
        thisUsedIngetCurrentScore &&
          thisUsedInaddPoints &&
          thisUsedIndeductPoints,
        'Have you used the `this` keyword for the `score` property within all three methods?'
      );

      test.assert(
        shoppinggame.player.getCurrentScore() == shoppinggame.player.score,
        "Have you returned the player's score from the method?"
      );
      shoppinggame.player.addPoints(10);
      test.assert(
        shoppinggame.player.score == 10,
        'Have you added the points to the current score of the player?'
      );

      shoppinggame.player.deductPoints(10);
      test.assert(
        shoppinggame.player.score == 0,
        'Have you subtracted the points from the current score of the player?'
      );
    });
  });
  ...
  describe('Define the Product object type', () => {
    it('Should define the Product object type. @Product-object', function () {
      test.assert(
        shoppinggame.Product,
        'Have you defined a `Product` object type?'
      );

      test.assert(
        prodNode,
        'Have you used the constructor function syntax to define `Product`?'
      );

      test.assert(
        prodNode.params.length == 4 &&
      ...
      test.assert(
        prodNode.body.body.find(
          (element) =>
            ...
            element.expression.right.name == 'id'
        ),
        'Have you initialized `id` property using a `this.id = id` statement within the constructor function?'
      );
      ...
      test.assert(
        prodNode.body.body.find(
          (element) =>
            ...
            element.expression.left.object &&
            element.expression.left.object.type == 'ThisExpression' &&
            element.expression.right.name == 'name'
        ),
        'Have you initialized `name` property using a `this.name = name` statement within the constructor function?'
      );
      ...
      test.assert(
        prodNode.body.body.find(
          (element) =>
            ...
            element.expression.left.object &&
            element.expression.left.object.type == 'ThisExpression' &&
            element.expression.right.name == 'price'
        ),
        'Have you initialized `price` property using a `this.price = price` statement within the constructor function?'
      );
      ...
      test.assert(
        prodNode.body.body.find(
          (element) =>
            ...
            element.expression.left.object &&
            element.expression.left.object.type == 'ThisExpression' &&
            element.expression.right.name == 'expiryDate'
        ),
        'Have you initialized `expiryDate` property using a `this.expiryDate = expiryDate` statement within the constructor function?'
      );
    });
  });
  ...
  describe('Implement the dateDiff() function', () => {
    it('Should implement the dateDiff() function to return the difference between two given dates. @dateDiff-function', function () {
      test.assert(
        shoppinggame.dateDiff,
        'Have you defined a function named `dateDiff()`?'
      );
      ...
      test.assert(
        ans,
        'Have you completed the `dateDiff()` function to return the difference between two given dates in number of days?'
      );
    });
  });
  ...
  describe('Add property daysToExpire to Product object type', () => {
    it('Should use Object.defineProperty() to define property daysToExpire. @Object-defineProperty', function () {
      test.assert(
        defineProperties,
        'Have you used `Object.defineProperty()` syntax to define a new property named `daysToExpire`?'
      );
    });
  });
  ...
  describe('Call dateDiff() function inside the getter of daysToExpire property of Product', () => {
    it('Should call and return the dateDiff function in the getter of daysToExpire. @dateDiff-in-get', function () {
      ...
      const thisUsed =
        daysToExpireCall &&
        daysToExpireCall.argument.arguments[0].object &&
        daysToExpireCall.argument.arguments[0].object.type === 'ThisExpression'
          ? true
          : false;
      const newUsed =
        daysToExpireCall &&
        daysToExpireCall.argument.arguments[1].type === 'NewExpression'
          ? true
          : false;

      test.assert(
        thisUsed && newUsed && pr.daysToExpire >= 0,
  ...
  describe('Add method getDetails() to Product object type', () => {
    it('Should add a method named getDetails() to Product that returns product details. @getDetails-method', function () {
      ...
      test.assert(
        pr1.getDetails,
        'Have you add a `getDetails()` method to `Product`?'
      );

      const retValue = pr1.getDetails();
      test.assert(
        retValue === 'Product Name: test1 , Product Price: 100',
        'Have you returned `Product Name: ${this.name} , Product Price: ${this.price}` from the method?'
      );
    });
  });
  ...
  describe('Define MagicProduct object type', () => {
    it('Should define the MagicProduct object type. @MagicProduct-object', function () {
      test.assert(
        magProdNode,
        'Have you used the constructor function syntax to define `MagicProduct`?'
      );

      test.assert(
        magProdNode.params.length == 6 &&
      ...
      const firstStmtIsCall =
        bodyLength > 0 && body[0].expression.type === 'CallExpression'
          ? true
          : false;

      test.assert(
        productCallInvocation && firstStmtIsCall,
        'Have you called the `Product` class constructor as the 1st statement inside `MagicProduct`'
      );
      ...
      const thisPassed =
        argsLength > 0 && arguments[0].type === 'ThisExpression' ? true : false;
      const otherPassed =
        arguments.find((element) => element.name == 'id') &&
        arguments.find((element) => element.name == 'name') &&
        arguments.find((element) => element.name == 'price') &&
        arguments.find((element) => element.name == 'expiryDate')
          ? true
          : false;

      test.assert(
        argsLength == 5 && thisPassed && otherPassed,
        'Have you passed `this, id, name, price, expiryDate` as parameters?'
      );
      ...
      test.assert(
        argsLength == 5 &&
          ...
          arguments[4].name == 'expiryDate',
        'Have you passed `this, id, name, price, expiryDate` in this order as parameters?'
      );

      const isBonus = body.filter(
        (element) =>
          element.expression.right && element.expression.right.name == 'isBonus'
      );
      const points = body.filter(
        (element) =>
          element.expression.right && element.expression.right.name == 'points'
      );

      test.assert(
        isBonus.length > 0 && points.length > 0,
        'Have you initialized `isBonus` & `points` properties?'
      );

      const thisExpressions = body.filter(
        (element) =>
      ...
      test.assert(
        thisExpressions.length == 2,
        'Have you used `this` to initialize `isBonus` & `points` properties?'
      );
    });
  });
  ...
  describe('Create the link (inheritance) between Product & MagicProduct object types', () => {
    it('Should make the MagicProduct a child of Product. @object-inheritance', function () {
      ...
      const hasLink =
        shoppinggame.MagicProduct.prototype instanceof shoppinggame.Product;

      test.assert(
        hasLink == true,
        'Have you made the `MagicProduct` a sub class of `Product`?'
      );
      ...
      test.assert(
        objectCreateCall,
        'Have you used `Object.create()`, passing `Product.prototype` as a parameter to link `MagicProduct` and `Product`?'
      );
    });
  });
  ...
  describe('Define Rating class', () => {
    it('Should define a class named Rating. @Rating-class', function () {
      test.assert(
        fieldDeclInRatingCls == false,
        'Have you done a Field declaration in the class body? It is an experimental feature according to MDN docs and we suggest you remove it.'
      );

      test.assert(
        ratingClassDecl,
        'Have you defined a class named `Rating` using the `class` keyword?'
      );
      ...
      for (let i = 0; i < classBody.length; i++) {
        if (
          ...
          classBody[i].value.body.body[0].expression.left.object.type ==
            'ThisExpression' &&
          classBody[i].value.body.body[0].expression.left.property.name ==
            'rate'
        ) {
      ...
      test.assert(
        constructorDefined,
        'Have you added a constructor that initializes a property named `rate` to a default value?'
      );

      //console.log(setterDefined);
      test.assert(
        setterDefined,
        'Have you added a setter that sets the value of a property named `rate`?'
      );
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        test.assert(
          r.rate != '',
          'Have you set the `rate` property value in the setter according to the given logic?'
        );

        test.assert(
          r.rate != '' && r.rate == rr.rate,
          'Have you set `this.rate` to the correct rate values?'
        );
      }
    });
  });
